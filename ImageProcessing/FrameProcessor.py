...
from ImageProcessing.DigitsPositionFinder import find_digits_positions
...
class FrameProcessor:
    ...
    def process_image(self, threshold_type, blur, threshold, adjustment, erode, iterations):

        debug_images, image_processed, image_processed_inverted = self.apply_image_filters(threshold_type, adjustment, blur, erode,
                                                                                           iterations, threshold)

        # Find the lcd digit contours
        contours, _ = cv2.findContours(image_processed_inverted, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)  # get contours
        ...
        # Use another method to get potential digits
        # potential_digits = find_digits_positions(image_processed)
        ...
        for pot_digit in potential_digits:
            ...
            cropped = image_processed[y:y + h, x: x + w]
    ...
    def apply_image_filters(self, threshold_type, adjustment, blur, erode, iterations, threshold):
        self.img = self.original.copy()
        debug_images = []
        alpha = float(2.5)
        ...
        cropped = img_blurred
        # Threshold the image
        cropped_threshold = cv2.adaptiveThreshold(cropped, 255, threshold_type, cv2.THRESH_BINARY_INV, threshold, adjustment)
        ...
        eroded = cv2.erode(cropped_threshold, kernel, iterations=iterations) if iterations != 0 else cropped_threshold
        debug_images.append(('Eroded', eroded))
        morph_close = cv2.morphologyEx(eroded, cv2.MORPH_CLOSE, kernel)
        debug_images.append(('Morprh close', morph_close))
        morph_close = cv2.morphologyEx(morph_close, cv2.MORPH_OPEN, kernel)
        debug_images.append(('Morprh open', morph_close))
        # Reverse the image to so the white text is found when looking for the contours
        re_inverse = inverse_colors(morph_close)
        debug_images.append(('Inversed', re_inverse))

        image_processed_inverted = morph_close
        image_processed = re_inverse

        return debug_images, image_processed, image_processed_inverted
